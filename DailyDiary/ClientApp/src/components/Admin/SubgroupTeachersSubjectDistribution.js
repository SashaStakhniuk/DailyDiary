...
class SubgroupTeachersSubjectDistribution extends React.Component { // розподілити викладачів в підгрупу по предметах
    ...
    componentDidMount() {
        ...
        if (this.props.allSubjects !== undefined) {
            this.setState({
    ...
    deleteSubjectFromTable = (e) => {
        ...
        */
        ...
        if (!hasDublicate && this.state.existingSubgroupTeachersSubjects.indexOf(x => x.subjectId == subjectId) != -1) { // якщо предмет один у таблиці (без повторів) і він є у списку вже розподілених
    ...
    appendRowInTable = async (teachersSubjectsDistribution) => {
        ...
        if (teachersSubjectsDistribution != undefined) {
            for (let i = 0; i < teachersSubjectsDistribution.length; i++) {
    ...
    onDistributionClick = () => {
        ...
        this.distributeTeachersBySubgroup(datasToSend)
    }
                ...
                <div className='d-flex flex-row justify-content-around'>
                    <div className="form-floating col-md">
                        <select id="subgroupBlock" name="subgroupBlock" className="form-select" onChange={(e) => this.onSubgroupBlockChange(e)} required>
                            ...
                            ) : <div style={{ color: "red" }}>SubgroupsBlocks not found</div>}
                        </select>
                    ...
                    <div className="form-floating col-md">
                        <select id="subgroup" name="subgroup" className="form-select" onChange={(e) => this.onSubgroupChange(e)} required>
                            ...
                            ) : <div style={{ color: "red" }}>Subgroups not found</div>}
                        </select>
...
//                 teacherId = '0';
//             }
//             arrayOfKeys.push({ "subjectId": subjectId, "additionalHours": hours, "teacherId": teacherId })
//         }
...
//             return 0;
//         }
//         else { // якщо такі існують
//             // let selectTeacher = document.createElement('select')
//             // selectTeacher.setAttribute('class', 'form-select');
//             var teachersList = '';
//             var hours = 0;
//             if (this.state.existingSubgroupTeachersSubjects.length > 0) {
//                 const subject = this.state.existingSubgroupTeachersSubjects.find(x => x.subjectId == e.target.value);
//                 if (subject !== undefined) {
//                     hours = subject.additionalHours
//                 }
//                 teachersList = `
//             ${teachers.map((teacher) =>
//                     this.state.existingSubgroupTeachersSubjects.find(x => x.teacherId == teacher.teacherId) != undefined ?
//                         `<option key={"teacher_"${teacher.teacherId}} selected value=${teacher.teacherId}>${teacher.name + " " + teacher.lastName}</option>`
//                         :
//                         `<option key={"teacher_"${teacher.teacherId}} value=${teacher.teacherId}>${teacher.name + " " + teacher.lastName}</option>`
//                 )}`
