...
class NewStudyPlan extends React.Component {
    ...
    async componentDidMount() {
        ...
        // this.setState({
        //     planTitle:`Навчальний план для усіх класів ${this.state.yearsOfStudy[0].yearStudy} року навчання ${new Date(this.state.studyYears[0].startYear).getYear() +" / "+ new Date(this.state.studyYears[0].finishYear).getYear()} навчального року`
        // })
                ...
                this.setState({
                    ...
                    currentPlanToDisplay:
                        data.subjectsToAdd.map(subjectFromDB => (
                            ...
                            this.arrayWithoutSelectedSubjects = this.arrayWithoutSelectedSubjects.filter(e => !this.selectedSubjects.find((id) => id == subjectFromDB.subjectId)),
